...
// didnt have time to implment as an angular solution
  ...
  socket.on("isTyping", function(data) {
    if (data.isTyping) {
      if ($("#"+data.person+"").length === 0) {
        $("#updates").append("<li id='"+ data.person +"'><span class='text-muted' style='color:#F5FFFA'><small><i class='fa fa-keyboard-o'></i> " + data.person + " is typing.</small></li>");
        timeout = setTimeout(timeoutFunction, 5000);
      }
    } else {
      $("#"+data.person+"").remove();
...
socket.on("history", function(data) {
  ...
  socket.on("update-people", function(data){
    ...
    $.each(data.people, function(a, obj) {
      ...
      $('#people').append("<li class=\"list-group-item\"><span style='color:red'>" + obj.name + "</span> <i class=\"fa fa-"+obj.device+"\"></i> " + html + " <a href=\"#\" class=\"whisper btn btn-xs\">whisper</a></li>");
    ...
    $("#rooms").append("<li class=\"list-group-item active\">List of rooms <span class=\"badge\">"+data.count+"</span></li>");
     if (!jQuery.isEmptyObject(data.rooms)) { 
      $.each(data.rooms, function(id, room) {
        ...
        $('#rooms').append("<li id="+id+" class=\"list-group-item\" style='color:red' ><span>" + room.name + "</span> " + html + "</li>");
      });
    } else {
      $("#rooms").append("<li class=\"list-group-item\" style='color:#4286f4'>There are no rooms yet.</li>");
    }
  });
